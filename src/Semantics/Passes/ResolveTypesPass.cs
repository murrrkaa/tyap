...
public sealed class ResolveTypesPass : AbstractPass
{
    ...
    public override void Visit(BinaryOperationExpression e)
    {
        ...
        {
            throw new TypeErrorException($"Оператор '{e.Operation}' не может быть применен к типам {e.Left.ResultType} и {e.Right.ResultType}");
        }
    ...
    public override void Visit(UnaryNotExpression e)
    {
        ...
        {
            throw new TypeErrorException($"Оператор '!' требует логического значения, получено {e.Operand.ResultType}");
        }
    ...
    private static ValueType ParseValueType(string? typeName)
    {
        ...
        return typeName.ToLowerInvariant() switch
        {
            ...
            "bool" => ValueType.Bool,
            "void" => ValueType.Void,
            _ => throw new TypeErrorException($"Неизвестное имя типа: {typeName}"),
        };
    }
